...
export async function GET(req: NextRequest) {
  const period = req.nextUrl.searchParams.get('period_days') ?? '30'
  try {
    const res = await fetch(`${ALEPH_API_URL}/api/portfolio/correlation?period_days=${period}`, { cache: 'no-store' })
